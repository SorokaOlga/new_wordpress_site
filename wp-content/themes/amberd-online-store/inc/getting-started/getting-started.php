    ...
    public function wpdevart_theme_info_welcome_admin_notice() {
        ...
        if ( is_admin() && !get_user_meta( get_current_user_id(), 'gs_notice_dismissed' ) ){
            echo '<div class="updated notice notice-success is-dismissible getting-started getting-started-wrap">';
                    echo '<div class="getting-content-wpdevart-admin">';
                        echo ( '<h2>' . sprintf( esc_html__( 'Welcome to %1$s Theme', 'amberd-online-store' ), esc_attr ( $theme->get( 'Name' ) ) ) . '</h2>' );
                        echo '<h3>To fully take advantage of the best our theme can offer, get started.</h3>';
                        echo ( '<p><a href="'.esc_url( admin_url( 'themes.php?page=amberd-online-store-about' ) ).'" class="button button-hero button-primary">' . sprintf( esc_html__( 'Get started with %s','amberd-online-store' ), esc_attr ( $theme->get( 'Name' ) ) ) . '</a>' );
                        echo ( '<a href="'.apply_filters( 'parent_amberd_premium_features_url', esc_url('https://wpdevart.com/amberd-wordpress-online-store-theme')).'" class="button button-hero button-outline" target="_blank">' . sprintf( esc_html__( '%s Premium','amberd-online-store' ), esc_attr ( $theme->get( 'Name' ) ) ) . '</a></p>' );
                        echo ( '<p class="plugin-notice">'.esc_html__( 'To get started, take action and learn more about our theme. If you want to hide this notice, click the Dismiss button in the top right corner.', 'amberd-online-store' ).'</p>' );
                    echo '</div>';
