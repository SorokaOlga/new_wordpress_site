...
function author_writer_customize_register( $wp_customize ) {
	...
	$wp_customize->add_control('author_writer_post_layout',array(
        ...
        'description' => __( 'Control Works only for full,left and right sidebar position in archieve posts', 'author-writer' ),
        'section' => 'author_writer_blog_option',
        'choices' => array(
	...
	$wp_customize->add_setting('author_writer_slider_top',array(
	    'default'=> '',
	    'sanitize_callback' => 'sanitize_text_field'
	));
	$wp_customize->add_control('author_writer_slider_top',array(
	    'label' => __('Add Slider Top Text','author-writer'),
	    'section'=> 'author_writer_slider_section',
	    'type'=> 'text'
	));
	...
	$wp_customize->add_setting('author_writer_slide_btn_2',array(
		'default'	=> '',
		'sanitize_callback'	=> 'esc_url_raw'
	));
	$wp_customize->add_control('author_writer_slide_btn_2',array(
		'label'	=> __('Second Button Link','author-writer'),
		'section'	=> 'author_writer_slider_section',
		'type'		=> 'url'
	));
...
if ( ! defined( 'AUTHOR_WRITER_PRO_THEME_URL' ) ) {
	define( 'AUTHOR_WRITER_PRO_THEME_URL', esc_url('https://www.themespride.com/products/author-writer-wordpress-theme'));
}
